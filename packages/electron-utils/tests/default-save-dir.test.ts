...
describe('resolveDefaultSaveDir', () => {
  ...
  it('creates and returns the fallback when nothing is configured', () => {
    const fallback = join(root, 'Documents', 'VibeOffice')
...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/VibeOffice without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'VibeOffice'))
    expect(existsSync(join(documents, 'VibeOffice'))).toBe(true)
  })
})
