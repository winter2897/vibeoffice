...
const tMain = createI18n({
  ...
  vi: {
    ...
    errParseFailed: 'Phân tích tệp không thành công',
    errImageNoText:
      'Tệp hình ảnh đính kèm không có văn bản; hình ảnh được gửi kèm theo tin nhắn của người dùng',
    errNotImage: 'không phải loại hình ảnh được hỗ trợ',
    errGskNotLoggedIn:
      'Chưa đăng nhập vào Aide: nhấp “Đăng nhập vào Aide” bên dưới, đăng nhập rồi thử lại',
    ...
    errDiskChanged: 'Bảng tính đã thay đổi trên đĩa sau khi mở — hãy dùng Lưu dưới dạng thay thế.',
    ...
    btnCancel: 'Hủy',
    csvSaveAsNotice:
      'Tệp CSV không thể giữ định dạng — lưu dưới dạng .xlsx sẽ giữ lại mọi thay đổi của bạn.',
  },
  'zh-TW': {
...
export function registerSheetsAiIpc(): void {
  ...
  // Aide account (gsk login state): the auth source for AI features; the
  ...
  ipcMain.handle(IPC_CHANNELS.aiStream, async (event, input: unknown) => {
    ...
    const provider = request.settings.provider as AiProviderId
    let config = request.settings.providers[provider]
    // Aide's key never enters the settings file; it is read from the gsk
